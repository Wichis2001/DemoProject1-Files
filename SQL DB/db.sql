...
SELECT e.nombre, SUM(dv.cantidad) as total_vendido FROM ControlVentas.DetalleVenta dv INNER JOIN ControlVentas.Venta v ON dv.id_venta = v.id_venta INNER JOIN ControlSucursal.Inventario inv ON dv.id_inventario = inv.id_inventario INNER JOIN ControlSucursal.Electrodomestico e ON inv.id_electrodomestico = e.id_electrodomestico WHERE inv.id_sucursal = 1 GROUP BY inv.id_electrodomestico, e.nombre ORDER BY total_vendido DESC LIMIT 5;

SELECT e.nombre, SUM(dv.precio) as total_vendido FROM ControlVentas.DetalleVenta dv INNER JOIN ControlVentas.Venta v ON dv.id_venta = v.id_venta INNER JOIN ControlSucursal.Inventario inv ON dv.id_inventario = inv.id_inventario INNER JOIN ControlSucursal.Electrodomestico e ON inv.id_electrodomestico = e.id_electrodomestico WHERE inv.id_sucursal = 1 GROUP BY inv.id_electrodomestico, e.nombre ORDER BY total_vendido DESC LIMIT 5;